...
const calculateTime = (inputDateStr) => {
    ...
    if (inputDate.getUTCDate() === currentDate.getUTCDate() &&
        ...
        const ampmTime = inputDate.toLocaleTimeString("en-US", { hour: "numeric", minute: "numeric" });
        return ampmTime;
    }
    else if (inputDate.getUTCDate() === currentDate.getUTCDate() - 1 &&
        inputDate.getUTCMonth() === currentDate.getUTCMonth() &&
        inputDate.getUTCFullYear() === currentDate.getUTCFullYear()) {
    ...
    else if (Math.floor((currentDate.valueOf() - inputDate.valueOf()) / (1000 * 60 * 60 * 24)) > 1 && Math.floor((currentDate.valueOf() - inputDate.valueOf()) / (1000 * 60 * 60 * 24)) <= 7) {
        //   const timeDifference = Math.floor( (currentDate - inputDate) / (1000 * 60 * 60 * 24));
        const timeDifference = Math.floor((currentDate.valueOf() - inputDate.valueOf()) / (1000 * 60 * 60 * 24));
